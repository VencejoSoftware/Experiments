...
begin
  ...
  begin
    ...
    circle(386, 186 +(19 * v), 4);
    circle(493, 186 +(19 * v), 4);
...
var
  ...
  hecho, nocambio, choque: boolean;
  Paos: array[1 .. 4] of PO;
begin
  hecho := false;
  nocambio := false;
  choque := false;
  if (StRot < MaxRot) then
  begin
    for c := 1 to 4 do
    begin
      AuX := Pos[c].X;
  ...
  for c := 1 to 4 do
    if Posicion[((Paos[c].Y + CmY)* 14)+((Paos[c].X + CmX))] = true then
      nocambio := true;

  if ((nocambio = false) and (hecho = false)) then
  begin
  ...
  if ((nocambio = true) and (hecho = false)) then
  begin
    {Hay choque y no reset ficha}
    if CmX > 11 then
    begin {Pared derecha}
      for c := 1 to 4 do
        if Posicion[((Paos[c].Y + CmY)* 14)+((Paos[c].X +(CmX - 1)))] = true then
          choque := true;
      if (choque = false) then
      begin
        for c := 1 to 4 do
        begin
          Pos[c].X := Paos[c].X;
  ...
  begin
    {Hay reset ficha}
    nocambio := false;
    if ((Fich = 2) and (CmX = 12)) then
    begin
      if (Posicion[(CmY * 14)+ 10] = false) then
      begin
        CmX := CmX - 1;
        MaxRot := EscogeFicha(Fich);
        StRot := 1;
      end;
      nocambio := true;
    end;
    if ((Fich = 4) and (CmX = 1)) then
    begin
      if ((Posicion[(CmY * 14)+ 2] = false) and (Posicion[(CmY * 14)+ 3] = false)
         and (Posicion[(CmY * 14)+ 4] = false)) then
      begin
        CmX := CmX + 1;
        MaxRot := EscogeFicha(Fich);
        StRot := 1;
    ...
    if ((Fich = 4) and (CmX = 12)) then
    begin
      if ((Posicion[(CmY * 14)+ 11] = false) and (Posicion[(CmY * 14)+ 10] = false)
         and (Posicion[(CmY * 14)+ 9] = false)) then
      begin
        CmX := CmX - 2;
        MaxRot := EscogeFicha(Fich);
        StRot := 1;
    ...
    begin
      ...
      choque := false;
      for c := 1 to 4 do
        if Posicion[((Pos[c].Y + CmY)* 14)+((Pos[c].X + CmX))] = true then
          choque := true;
      if (choque = true) then
      begin
        for c := 1 to (MaxRot - 1) do
        begin
          for l := 1 to 4 do
          begin
            AuX := Pos[l].X;
...
begin
  ...
  line(60, 20, 580, 20);
  setcolor(0);
  line(220, 70, 260, 70); {Lineas quitaladrillos}
  line(340, 20, 380, 20);
  line(480, 460, 520, 460);
  line(440, 430, 480, 430);
  line(60, 411, 60, 431);
  setfillstyle(6, 4);
  floodfill(1, 1, 4); {Fondos}
  setcolor(12);
  setfillstyle(1, 0); {Lineas exteriores del tablero}
  bar(101, 71, 299, 398);
  bar(361, 171, 519, 429);
  setfillstyle(6, 4);
  floodfill(150, 300, 4);
...
begin
  setcolor(12);
  setfillstyle(1, 0);
  bar(105, 80, 288, 398); {Tapa fondos}
  bar(365, 175, 514, 424);
  line(365, 175, 365, 424);
  line(365, 424, 514, 424);
  line(514, 424, 514, 175);
  line(105, 80, 105, 398);
  line(105, 398, 288, 398);
  line(288, 398, 288, 80);
end;
...
begin
  ...
  setcolor(11);
  outtextxy(385, 190, 'ELIJA HANDICAP');
  setcolor(8);
  line(385, 210, 495, 210);
  nl := 0;
  Selec(0, 6);
  repeat
    tl := readkey;
...
begin
  ...
  case f of
    ...
    4:
      begin
        rectangle(468, 270, 473, 275);
        rectangle(473, 270, 478, 275);
        rectangle(483, 270, 488, 275);
        rectangle(478, 270, 483, 275);
      end;
    5:
      begin
        rectangle(480, 270, 485, 275);
...
{$I-} rewrite(PUF); {$I+}
  ...
  begin
    closegraph;
    writeln('­ERROR!, No se pudo crear el fichero de datos.');
    halt;
  end;
  for s := 1 to 7 do
  begin
...
begin
  PasaPersiana;
  setcolor(11);
  outtextxy(392, 190, 'HALL OF FAME');
  setcolor(8);
  line(385, 210, 495, 210);
  setcolor(7);
  setfillstyle(9, 8);
  ...
  setcolor(8);
  line(448, 264, 448, 256);
  line(401, 264, 401, 259);
  m := imagesize(374, 248, 501, 265);
  getmem(s, m);
  getimage(374, 248, 501, 265, s^);
  putimage(374, 277, s^, 0);
  putimage(374, 219, s^, 0);
  putimage(374, 306, s^, 0);
  putimage(374, 335, s^, 0);
  putimage(374, 364, s^, 0);
  putimage(374, 393, s^, 0);
  freemem(s, m);
  if (datosenmem = false) then
    ManejaDatos(true);
  for j := 1 to 7 do
  begin
    str(j, l);
    setcolor(9);
    outtextxy(373, 191 +(29 * j), l);
    setcolor(15);
    outtextxy(390, 188 +(29 * j), Datos[j].Nombre);
    str(Datos[j].Nivel, l);
    setcolor(7);
    outtextxy(405, 199 +(29 * j), l);
    str(Datos[j].Points, l);
    outtextxy(453, 199 +(29 * j), l);
  end;
  setcolor(8);
  outtextxy(405, 413, 'Nivel');
  outtextxy(453, 413, 'Puntos');
  line(401, 410, 401, 421);
  line(448, 410, 448, 421);
  readkey;
end;
...
begin
  PasaPersiana;
  setcolor(11);
  outtextxy(392, 190, 'NUEVO RECORD');
  setcolor(8);
  line(385, 210, 495, 210);
  setcolor(7);
  outtextxy(388, 230, '­Enhorabuena!');
  ...
  bar(389, 349, 491, 359);
  setcolor(15);
  outtextxy(404, 310, 'Introduce');
  outtextxy(404, 330, 'tu nombre');
  repeat
    t := readkey;
    if ((t <> #13) and (t <> #8) and (p < 13)) then
    begin
      juli[p] := t;
      setcolor(9);
      outtextxy(385 +(8 * p), 351, juli[p]);
    end;
    if ((t = #8) and (p > 1)) then
    begin
      p := p - 1;
  ...
  for p := 1 to 7 do
    for s := p + 1 to 7 do
      if ((Datos[p].Points < Datos[s].Points) or
         ...
         and (Datos[p].Nivel < Datos[s].Nivel))) then
      begin
        AuX := Datos[p].Points;
        Datos[p].Points := Datos[s].Points;
        Datos[s].Points := AuX;
...
begin
  if (datosenmem = false) then
    ManejaDatos(true);
  for c := 1 to 7 do
    if Ptos > Datos[c].Points then
    begin
      NuevoRecord(Ptos, Nvel);
      datosenmem := true;
      break;
    end;
  VerPuntuaci;
...
begin
  setcolor(8);
  line(372, 204, 490, 204);
  line(372, 183, 511, 183);
  line(372, 225, 469, 225);
  rectangle(489, 213, 502, 226);
  line(448, 246, 511, 183);
  line(460, 243, 450, 253); {diagonales}
  line(372, 246, 448, 246);
  line(372, 183, 372, 246);
  line(505, 243, 505, 290);
  line(505, 243, 460, 243);
  line(450, 253, 450, 290);
  line(450, 290, 505, 290);
  setcolor(7);
  ...
  begin
    setcolor(10);
    outtextxy(492, 216, #14);
  end;
  setcolor(9); {Mostrar resultados conseguidos}
  str(lih, numli);
  outtextxy(436, 211, numli);
  str(niv - 1, numli);
  outtextxy(436, 232, numli);
  str(Pos, numli);
  outtextxy(436, 190, numli);
  setcolor(11);
  outtextxy(385, 260, 'Tetris');
  setcolor(1);
  setfillstyle(1, 1);{Pintado de fichas 3x3}
  bar(391, 407, 394, 416);
  bar(388, 413, 391, 416);
  setcolor(2);
  setfillstyle(1, 2);
...
begin
  ...
  closegraph;
  writeln('TETRIS - V¡ctor Barbero - Dic. 2.001');
  halt;
end;
...
begin
  setcolor(8);
  outtextxy(385, 180, 'V¡ctor Barbero');
  outtextxy(393, 358, 'Dic. - 2.001');
  s := imagesize(382, 177, 497, 188);
  getmem(p, s);
  t := imagesize(391, 355, 488, 365);
  getmem(l, t);
  getimage(382, 177, 497, 188, p^);
  getimage(391, 355, 488, 365, l^);
  for c := 177 to 260 do
  begin
    putimage(382, c, p^, 0);
    Delay(15 * DLY);
    putimage(391,(260 - c)+ 272, l^, 0);
  end;
  freemem(p, s);
  freemem(l, t);
  borrado := false;
  ...
  until X < 0;
  setcolor(11);
  outtextxy(471, 204, 'v1.0');
  setcolor(7);
  outtextxy(389, 340, 'Nueva partida');
...
begin
  ...
  nvl := nvl + 1;
  setcolor(11);
  outtextxy(380, 190, 'CAMBIO DE NIVEL');

  setcolor(8);
  line(385, 210, 495, 210);
  line(372, 312, 507, 312);
  line(372, 399, 507, 399);
  outtextxy(425, 328, 'x500');
  outtextxy(425, 348, 'x350');
  outtextxy(425, 368, 'x225');
  outtextxy(425, 388, 'x100');

  setcolor(7);
  outtextxy(387, 220, '­Enhorabuena!');
  outtextxy(412, 235, 'pasa al');
  outtextxy(418, 403, 'TOTAL');
  outtextxy(409, 258, 'Nivel');
  outtextxy(375, 300, 'Lineas');
  outtextxy(457, 300, 'Puntos');
  outtextxy(397, 320, 'Tetris');
  outtextxy(397, 340, 'Triple');
  outtextxy(397, 360, 'Double');
  outtextxy(397, 380, 'Single');

  setcolor(4);
  line(400, 253, 474, 253);
  line(400, 270, 474, 270);
  line(398, 251, 476, 251);
  line(398, 272, 476, 272);

  setcolor(9);
  str(nvl - 1, s);
  outtextxy(459, 258, s);
  str(LinH[4], s);
  outtextxy(378, 320, s); {Lineas hechas}
  t := - 5;
  repeat
    t := t + 5;
    str(t, s);
  ...
  Delay(100 * DLY);
  str(LinH[3], s);
  outtextxy(378, 340, s); {Lineas hechas}
  if sonido then
  begin
    Sound(880);
    Delay(8 * DLY);
  ...
  Delay(100 * DLY);
  str(LinH[2], s);
  outtextxy(378, 360, s); {Lineas hechas}
  if sonido then
  begin
    Sound(880);
    Delay(8 * DLY);
  ...
  Delay(100 * DLY);
  str(LinH[1], s);
  outtextxy(378, 380, s); {Lineas hechas}
  if sonido then
  begin
    Sound(880);
    Delay(8 * DLY);
  ...
  VaciaBuffer; {Int_10h, eres ganador!!}
  readkey;
  PasaPersiana;
  PoneMarcador(LineasH, nvl, Ptos);
  setfillstyle(1, 0);
  setcolor(0); {ReSet del Campo}
  bar(106, 80, 287, 397);
  for t := 1 to 293 do
    if Posicion[t] = true then
      PintaCuadro(t mod 14, t div 14, nvl);
end;
...
var
  ...
  begin
    setcolor(col);
    rectangle(379, 237 +(opt * 23), 382, 240 +(opt * 23));
...
begin
  PasaPersiana;
  setcolor(11);
  outtextxy(426, 190, 'MENU');
  setcolor(8);
  line(385, 210, 495, 210);
  setcolor(7);
  if not sonido then
    outtextxy(385, 260, 'Activar sonido.')
  else
    outtextxy(385, 260, 'Quitar sonido.');
  ...
  repeat
    t := readkey;
    if ((t = #80) and (op < 4)) then
    begin
      Selecta(op, 0);
      op := op + 1;
      Selecta(op, 10);
    end;
    if ((t = #72) and (op > 1)) then
    begin
      Selecta(op, 0);
      op := op - 1;
      Selecta(op, 10);
  ...
  case op of
    ...
    4:
      begin
        ...
        if t = 'S' then
        begin
          CompruebaMejora(Puntos, Nivel);
          FinJuego;
        end;
      end;
  end;
  PasaPersiana;
  PoneMarcador(LineasH, Nivel, Puntos);
end;
...
var
  ...
  var
    p: integer;
  begin
    for p := (BLinea * 14)+ 14 downto 15 do
    begin {CAMBIO DE POSICIONES}
      if p > (BLinea * 14) then
      begin {Pintado de la linea a bajar}
        if Posicion[p - 14] = true then
        begin
      ...
      if p < (BLinea * 14) then
      begin {Pintado de las lineas anteriores}
  ...
  begin
    ...
    begin {Linea TETRIS}
      ...
      begin
        setcolor(4);
        line(p, 83 +(15 * LGanadoras[1]), p, 96 +(LGanadoras[4]* 15));
      end;
      if p = 174 then
      begin
        setcolor(15);
        ...
        outtextxy(159,(87 + 15 * LGanadoras[2]), 'T');
      end;
      if p = 189 then
      begin
        setcolor(15);
        ...
        outtextxy(174,(87 + 15 * LGanadoras[2]), 'E');
      end;
      if p = 204 then
      begin
        setcolor(15);
        ...
        outtextxy(189,(87 + 15 * LGanadoras[2]), 'T');
      end;
      if p = 219 then
      begin
        setcolor(15);
        ...
        outtextxy(204,(87 + 15 * LGanadoras[2]), 'R');
      end;
      if p = 234 then
      begin
        setcolor(15);
        ...
        outtextxy(219,(87 + 15 * LGanadoras[2]), 'I');
      end;
      if p = 249 then
      begin
        setcolor(15);
        ...
        outtextxy(234,(87 + 15 * LGanadoras[1]), 'S');
      end;
      if p > 178 then
      begin
        setcolor(0);
    ...
    BajaLineas(LGanadoras[2]);
    BajaLineas(LGanadoras[3]);
    BajaLineas(LGanadoras[4]);
    NoSound;
  end;
  ...
  begin
    ...
    begin {Linea TRIPLE}
      ...
      begin
        ...
        line(p, 83 +(15 * LGanadoras[3]), p, 97 +(LGanadoras[3]* 15));
      end;
      if p = 174 then
      begin
        setcolor(15);
      ...
      begin
        ...
        outtextxy(174,(87 + 15 * LGanadoras[2]), 'R');
      end;
      if p = 204 then
      begin
        setcolor(15);
        ...
        outtextxy(189,(87 + 15 * LGanadoras[2]), 'I');
      end;
      if p = 219 then
      begin
        setcolor(15);
        ...
        outtextxy(204,(87 + 15 * LGanadoras[2]), 'P');
      end;
      if p = 234 then
      begin
        setcolor(15);
        ...
        outtextxy(219,(87 + 15 * LGanadoras[2]), 'L');
      end;
      if p = 249 then
      begin
        setcolor(15);
        ...
        outtextxy(234,(87 + 15 * LGanadoras[1]), 'E');
      end;
      if p > 178 then
      begin
        setcolor(0);
        ...
        line(p - 71, 83 +(15 * LGanadoras[3]), p - 71, 97 +(LGanadoras[3]* 15));
      end;
      Delay(6 * DLY);
      if sonido then
        Sound(p * 3);
  ...
  begin
    ...
    begin {Linea DOUBLE}
      ...
      begin
        ...
        outtextxy(159,(87 + 15 * LGanadoras[2]), 'D');
      end;
      if p = 189 then
      begin
        setcolor(15);
        outtextxy(174,(87 + 15 * LGanadoras[1]), 'O');
        outtextxy(174,(87 + 15 * LGanadoras[2]), 'O');
      end;
      if p = 204 then
      begin
        setcolor(15);
        outtextxy(189,(87 + 15 * LGanadoras[1]), 'U');
        outtextxy(189,(87 + 15 * LGanadoras[2]), 'U');
      end;
      if p = 219 then
      begin
        setcolor(15);
        outtextxy(204,(87 + 15 * LGanadoras[1]), 'B');
        outtextxy(204,(87 + 15 * LGanadoras[2]), 'B');
      end;
      if p = 234 then
      begin
        setcolor(15);
  ...
  begin
    ...
    begin {Linea SINGLE}
      ...
      begin
        setcolor(4);
        line(p, 83 +(15 * LGanadoras[1]), p, 96 +(LGanadoras[1]* 15));
      end;
      if p = 174 then
      begin
        setcolor(15);
        outtextxy(159,(87 + 15 * LGanadoras[1]), 'S');
      end;
      if p = 189 then
      begin
        setcolor(15);
        outtextxy(174,(87 + 15 * LGanadoras[1]), 'I');
      end;
      if p = 204 then
      begin
        setcolor(15);
        outtextxy(189,(87 + 15 * LGanadoras[1]), 'N');
      end;
      if p = 219 then
      begin
        setcolor(15);
        outtextxy(204,(87 + 15 * LGanadoras[1]), 'G');
      end;
      if p = 234 then
      begin
        setcolor(15);
      ...
      begin
        setcolor(0);
        line(p - 91, 83 +(15 * LGanadoras[1]), p - 91, 96 +(LGanadoras[1]* 15));
      end;
      Delay(4 * DLY);
      if sonido then
        Sound(p);
...
begin
  ...
  repeat
    if ((Posicion[l]) and (Posicion[l + 1]) and
       ...
       (Posicion[l + 11])) then
    begin
      LGanadoras[v] := l div 14;
      v := v + 1;
    end;
    l := l + 14;
  until l > 285;
  setcolor(15);
  if ((LGanadoras[4] <> 0) and (ya = true)) then
    LineaTetris;
  if ((LGanadoras[3] <> 0) and (ya = true)) then
    LineaTriple;
  if ((LGanadoras[2] <> 0) and (ya = true)) then
    LineaDoble;
  if ((LGanadoras[1] <> 0) and (ya = true)) then
    LineaSimple;
  if ya = false then
  begin
    setcolor(0);
    setfillstyle(1, 0); {Borrar lineas anteriores}
    bar(434, 210, 460, 220);
...
var
  CdX, CdY: shortint;
  avnzY, Rotacion, nocambio: boolean;
begin {1 -> CdmX++; -1 -> CdmX--; 2 ->RotaFicha; 3 ->CdmY++;}
  ...
  if Posicion[((Pos[1].Y + CdY)* 14)+((Pos[1].X + CdX))] = true then
    nocambio := true;
  if Posicion[((Pos[2].Y + CdY)* 14)+((Pos[2].X + CdX))] = true then
    nocambio := true;
  if Posicion[((Pos[3].Y + CdY)* 14)+((Pos[3].X + CdX))] = true then
    nocambio := true;
  if Posicion[((Pos[4].Y + CdY)* 14)+((Pos[4].X + CdX))] = true then
    nocambio := true;
  if ((nocambio = false) and (Rotacion = false)) then
  begin
    BorraFicha(CmmX, CmmY);
    case Vx of
      1:
...
begin
  PasaPersiana;
  setcolor(4);
  line(402, 210, 476, 210);
  line(402, 210, 402, 255);
  line(403, 255, 475, 255);
  line(476, 211, 476, 255);
  line(396, 204, 482, 204);
  line(396, 205, 396, 261);
  line(397, 261, 482, 261);
  line(482, 260, 482, 205);
  setfillstyle(6, 4);
  floodfill(440, 207, 4);
  setcolor(11);
  outtextxy(412, 220, 'G A M E');
  Delay(300 * DLY);
  outtextxy(412, 240, 'O V E R');
  Delay(300 * DLY);
  setcolor(7);
  ...
  setcolor(9);
  str(putos, cadn);
  outtextxy(465, 290, cadn);
  str(Lin, cadn);
  outtextxy(465, 310, cadn);
  setcolor(7);
  outtextxy(392, 345, 'šDesea jugar');
  outtextxy(379, 363, 'otra vez? [S/N]');
  repeat
    t := readkey;
    t := upcase(t);
  until t in ['S', 'N'];
...
begin
  ...
  case PosP of
    #77:
      AvanzaFicha(1, afiu, CX, CY);
    #75:
      AvanzaFicha(- 1, afiu, CX, CY);
    #32:
      AvanzaFicha(2, afiu, CX, CY);
    #80:
      Rts := 0;
    #27:
      Menu;
...
begin
  ...
  setfillstyle(1, 0);
  bar(434, 189, 485, 197); {Borrar los puntos anteriores}
  setcolor(9);
  str(Pts, nml);
  outtextxy(436, 190, nml);
...
begin
  ...
  sonido := true;
  IniciaVideo;
  IniciaCampo; {Pintado del tablero}
  repeat
    ReiniciaCampo;
    if (primejec = true) then
    begin
      Presentacion;
      primejec := false;
      PasaPersiana;
    ...
    CdmY := EligeHandicap;
    NivelInic := Nivel;
    for Puntos := 1 to 308 do
      Posicion[Puntos] := false; {Borrado de arrays}
    for CdmX := 0 to CdmY do
    begin {Activación de las casillas de Handicap}
      for Ficha := 0 to 12 do
      begin
        if ((random(10) > 3)) then
          Posicion[((21 - CdmX)* 14)+ Ficha] := true;
      end;
    end;
    for LineasH := 1 to 294 do
      if Posicion[LineasH] then
        PintaCuadro(LineasH mod 14, LineasH div 14, Nivel);
    Puntos := 0;
    while Puntos < 293 do
    begin {Declaración de bordes en Posicion}
      Posicion[Puntos] := true;
      Posicion[Puntos + 13] := true;
      Puntos := Puntos + 14;
    end;
    for Puntos := 294 to 308 do
      Posicion[Puntos] := true;
    ...
    Ficha := EscAlFich(NextFich);
    StRot := 1;
    MaxRot := EscogeFicha(Ficha); {Preparación de fichas}
    CdmX := 7;
    CdmY := 1;
    PintaFicha(CdmX, CdmY, Ficha);
    repeat
      ...
      begin
        ...
        if CdmY < 2 then
          TanMatao(Puntos, LineasH)
        else
        begin
          FichUs[NextFich] := FichUs[NextFich] + 1;
          SubFichUs(NextFich, FichUs[NextFich]);
          Ficha := EscAlFich(NextFich);
          MaxRot := EscogeFicha(Ficha);
          CdmX := random(5)+ 3;
          StRot := 1;
          CdmY := 1;
          PintaFicha(CdmX, CdmY, Ficha);
